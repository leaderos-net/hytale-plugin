...
public class CreditSetSubCommand extends AbstractAsyncCommand {
    private final RequiredArg<String> playerArg;
    private final RequiredArg<Double> amountArg;

    public CreditSetSubCommand() {
        super("set", "Sets the credit amount of a player.");

        addAliases("ayarla");

        playerArg = withRequiredArg("player", "Target player", ArgTypes.STRING);
        amountArg = withRequiredArg("amount", "Amount of credits to set", ArgTypes.DOUBLE);
    }
    ...
    protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context) {
        ...
        CommandUtil.requirePermission(sender, HytalePermissions.fromCommand("leaderos.credit.set"));
        ...
        return runAsync(context, () -> {
            ...
            if (Objects.requireNonNull(setCreditResponse).getResponseCode() == HttpURLConnection.HTTP_OK) {
                ChatUtil.sendMessage(sender, ChatUtil.replacePlaceholders(
                        LeaderosPlugin.getInstance().getLangFile().getMessages().getCredit().getSuccessfullySetCredit(),
